# Personal & Academic Projects

## Overview

A curated collection of projects from personal interests and academic coursework.

## Table Of Contents

- [Algorithms and Data Structures](https://github.com/atahancetindemir/projects/tree/main/algorithms-data-structures)
- [Debugging](https://github.com/atahancetindemir/projects/tree/main/debugging)
- [Embedded Systems](https://github.com/atahancetindemir/projects/tree/main/embedded-systems)
- [Machine Learning](https://github.com/atahancetindemir/projects/tree/main/machine-learning)
- [Math Puzzles](https://github.com/atahancetindemir/projects/tree/main/math-puzzles)
- [Number Theory](https://github.com/atahancetindemir/projects/tree/main/number-theory)
- [Numerical Analysis](https://github.com/atahancetindemir/projects/tree/main/numerical-analysis)
- [Optimization](https://github.com/atahancetindemir/projects/tree/main/optimization)
- [Probabilitiy and Statistics](https://github.com/atahancetindemir/projects/tree/main/probability-statistics)

## Project Categories

### Algorithms and Data Structures

- **Arrays**
- **Matrices**
- **Linked Lists**
- **Prime Numbers**
- **Fibonacci Numbers**

### Debugging

- **Debugger Structure**
- **Delta Debugger**
- **Cause Chain Effect**

### Embedded Systems

- **Event-Triggered Scheduler**
- **Static Scheduler**
- **Tetris Game Using SSD 1308**

### Machine Learning

- **Prediction Model**

### Math Puzzles

- **Striling's Approximation**
- **Earthquake Energy Calculation**
- **Car and Fly Problem**

### Number Theory

- **Collatz Conjecture**
- **Dudeney Numbers**

### Numerical Analysis

- **Euler's Method**
- **Heuns's Method**
- **Methods Compared To The Analytic Solution**
- **Numerical Differantiation**
- **Numerical Integration**

### Optimization

- **Cache Friendly Matrix Multiplication**
- **Loop Unrolling**
- **Row/Column Acces Patterns**
- **Vectorized Strassen's Matrix Multiplication** 

### Probability and Statistics

- **Monte-Carlo Simulations**
- **Financial Calculations**
- **Sequence Experiments**
- **Probability Problems**
- **Expected Lottery Probability**